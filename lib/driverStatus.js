...
_locationSub = await Location.watchPositionAsync(
  {
    accuracy: Location.Accuracy.Balanced, // ✅ cheaper battery; keep High if you insist
    timeInterval: 7000,                   // ✅ request ~7s updates (OS may throttle)
    distanceInterval: 0,                  // ✅ allow callbacks even when not moving (traffic)
  },
    ...
    async (loc) => {
      try {
        ...
        if (now - _lastSentAt < 7000) return; // ✅ hard limit: 1 write per 7s max
